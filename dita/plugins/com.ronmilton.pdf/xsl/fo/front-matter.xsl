...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="createFrontMatter">
    <xsl:if test="$generate-front-cover">
            ...
            <xsl:call-template name="createFrontCoverContents"/>
              <fo:block-container xsl:use-attribute-sets="__frontmatter__product__container">
                <fo:block xsl:use-attribute-sets="__frontmatter__product">
                  ...
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$ds.productVersion"/>
                </fo:block>
              </fo:block-container>
              <fo:block-container xsl:use-attribute-sets="__frontmatter__address__container">
                <fo:block xsl:use-attribute-sets="__frontmatter__address">
                  <xsl:value-of select="$ds.company"/>
                </fo:block>
                <fo:block xsl:use-attribute-sets="__frontmatter__address">
                  <xsl:value-of select="$ds.address"/>
                </fo:block>
                <fo:block xsl:use-attribute-sets="__frontmatter__address">
                  <xsl:value-of select="$ds.citystatezip"/>
                </fo:block>
                <fo:block xsl:use-attribute-sets="__frontmatter__address">
                  <fo:basic-link external-destination="url('{$ds.support-email}')"><xsl:value-of select="$ds.support"/></fo:basic-link>
                </fo:block>
              </fo:block-container>
          </fo:block-container>
        </fo:flow>
      ...
      <fo:page-sequence master-reference="front-matter" xsl:use-attribute-sets="page-sequence.cover">
        <fo:flow flow-name="xsl-region-body">
          <fo:block-container xsl:use-attribute-sets="__frontmatter__legal">
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__heading">
              <xsl:text>Copyright Notice</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__body">
              <xsl:text>Copyright&#169; </xsl:text>
              <xsl:value-of select="format-dateTime(current-dateTime(), '[Y]')"></xsl:value-of> 
              <xsl:text> Front matter legal content</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__heading">
              <xsl:text>Trademarks</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__body">
              <xsl:text>Trademark legal content.</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__heading">
              <xsl:text>Confidentiality</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__body">
              <xsl:text>This document is configential. Only people with an internet connection may view it.</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__heading">
              <xsl:text>Disclaimer</xsl:text>
            </fo:block>
            <fo:block xsl:use-attribute-sets="__frontmatter__legal__body">
              <xsl:text>The information contained in this document is intended to describe nothing in particular.</xsl:text>
            </fo:block>
          </fo:block-container>
        </fo:flow>
      </fo:page-sequence>
  ...
  <xsl:template name="cover-image">
    <fo:block text-align="center" width="100%" margin-top="0">
